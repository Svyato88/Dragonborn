...
public class NBTBlockCheckFeature extends Feature<NBTBlockCheckConfiguration> {
    public NBTBlockCheckFeature(Codec<NBTBlockCheckConfiguration> codec) {
        super(codec);
    }
    ...
    @Override
    public boolean place(FeaturePlaceContext<NBTBlockCheckConfiguration> context) {
        WorldGenLevel level = context.level();
        BlockPos origin = context.origin();
        BlockPos belowOrigin = origin.below();
        BlockState belowOriginState = level.getBlockState(belowOrigin);
        RandomSource random = context.random();
        NBTBlockCheckConfiguration config = context.config();
        ...
        if (!config.validBlocks.test(belowOriginState, random)) {
            Dragonborn.LOGGER.warn("[NBTStructureFeature] Structure cannot be placed in {} on pos {}!", belowOriginState,belowOrigin);
            return false;
        }
